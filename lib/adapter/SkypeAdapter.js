...
class SkypeAdapter extends BaseAdapter {
    ...
    initializeBotbuilders(configAry){
        ...
        configAry.forEach((setting) => {
            ...
            bot.on('conversationUpdate', function (message) {
                ...
                console.log('skype conversation update',message);
                ...
                if (message.action === 'add') {
                    ...
                    setTimeout(() => {
                        callBackInfo.callBack(
                            ...
                            this.generateOpenChatObj(callBackInfo.service,message),
                            session.message
                        );
                    },10);
            ...
            bot.dialog('/',(session) => {
                ...
                if(callBackInfo && callBackInfo.callBack){
                    ...
                    this.userIdentifierPool[session.message.user.id] = session.message.address;
    ...
    processPost(req,res,service,cb){
        ...
        console.log("skype web hook",data);
        ...
        console.log('service.config.appId',service.config.appId);
        ...
        console.log('this.messageCallbackPool[req.body.id]',this.messageCallbackPool[req.body.id]);
    ...
    generateOpenChatObj(service,message){

        const messageObj = {
            userIdentifier:service.provider + ":" + service.identifier + ":" + message.from.id,
            providerIdentifier:service.identifier,
            provider:service.provider,
            providerUserId:session.message.from.id
        };
    ...
    generateMessageObj(service,session){
        ...
        const message = {
            userIdentifier:service.provider + ":" + service.identifier + ":" + session.message.user.id,
            providerIdentifier:service.identifier,
            provider:service.provider,
            providerUserId:session.message.user.id
        };
    ...
    replyText(text,message,service){

        return new Promise((resolve,reject) => {
            
            const messageId = message.address.id;
    ...
    sendText(text,userIdentifier,service){
        ...
        return new Promise((resolve,reject) => {
            ...
            const userId = userIdentifier.replace(prefix,"");
            const address = this.userIdentifierPool[userId];
